...
comments.  Compares issues by a stable key (file, rule, message) so line-only
shifts from unrelated edits are treated as the same issue.

Adapted from github.com/NousResearch/hermes-agent (Apache-2.0).
"""
...
def _normalize_ruff(entries: list[dict], cwd: Path | None = None) -> list[dict]:
    ...
    for e in entries:
        ...
        try:
            filename = os.path.relpath(filename, start=cwd)
        except ValueError:
            pass
...
def _key(d: dict) -> tuple[str, str, str]:
    """Stable diagnostic identity: (path, rule, message)."""
...
def _tool_report(
    ...
    emoji = "\U0001f195" if delta > 0 else ("✅" if delta < 0 else "➖")
    ...
    lines.append(_section("\U0001f195 New issues", new))
...
def main() -> int:
    ...
    base_ruff = _normalize_ruff(base_ruff_raw, cwd=args.base_dir)
